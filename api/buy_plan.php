...
function utf8_first_char(string $s): string
{
...
function utf8_upper(string $s): string
{
...
function tg_send(string $token, string $chat, string $txt, string $parse = 'HTML'): bool
{
  ...
  if (!function_exists('curl_init')) {
    logerr('curl missing for telegram');
    return false;
  }
  ...
  if (!$ok)
    logerr('tg_send fail: ' . curl_error($ch) . ' http=' . curl_getinfo($ch, CURLINFO_HTTP_CODE));
  curl_close($ch);
  return $ok;
}
function mask_public_name(string $who): string
{
  $name = trim(ltrim($who, '@'));
  if ($name === '')
    return 'U***';
  $f = utf8_first_char($name);
  return utf8_upper($f) . '***';
}
function make_receipt_num(int $len = 10): string
{
...
function mask_receipt_numeric(string $num): string
{
  $n = preg_replace('/\D+/', '', $num);
  $L = strlen($n);
  if ($L <= 3)
    return $n;
  return substr($n, 0, 1) . str_repeat('X', max(0, $L - 3)) . substr($n, -2);
}
function fmt_human_date(string $ymd): string
{
  try {
    $d = new DateTime($ymd);
...
function plain_plan_name(string $label): string
{
  $s = preg_replace('/[^\p{L}\p{N}\s\-]/u', '', $label);
...
try {
  ...
  if ($action === '' || $action === 'plan') {
    ...
    if ($planKey === '' || !isset($PLANS[$planKey])) {
      echo json_encode(['ok' => false, 'error' => 'NO_PLAN']);
      exit;
    }
    $plan = $PLANS[$planKey];
    ...
    $haveX = (int)$u['xbal'];
    if ($haveX < $price) {
      echo json_encode(['ok' => false, 'error' => 'INSUFFICIENT']);
      exit;
    }
    // expiry compute
    ...
    if (!empty($u['expiry_date'])) {
      try {
        $cur = new DateTime((string)$u['expiry_date']);
...
➜ BabaChecker.com";
  ...
  if ($action === 'credits') {
    ...
    if ($packKey === '' || !isset($CREDIT_PACKS[$packKey])) {
      echo json_encode(['ok' => false, 'error' => 'NO_PACK']);
      exit;
    }
    ...
    if ($hasCreditsExpiry && !empty($u['credits_expiry'])) {
      try {
        ...
        if ($cur > $now)
          $newCreditsExpiry = max($newCreditsExpiry, $u['credits_expiry']);
      }
      catch (Throwable) {
      }
    }
...
➜ BabaChecker.com";
  ...
  // ---------------- KILLER CREDIT PACKS ----------------
  if ($action === 'kcoin') {
    if (!$hasK || !$hasKcoinExpiry) {
      echo json_encode(['ok' => false, 'error' => 'UNSUPPORTED']);
      exit;
    }
    $packKey = isset($_POST['pack']) ? strtolower(trim((string)$_POST['pack'])) : '';
    if ($packKey === '' || !isset($KILLER_PACKS[$packKey])) {
      echo json_encode(['ok' => false, 'error' => 'NO_PACK']);
      exit;
    }
    ...
    if (!empty($u['kcoin_expiry'])) {
      try {
        ...
        if ($cur > $now)
          $newKcoinExpiry = max($newKcoinExpiry, $u['kcoin_expiry']);
      }
      catch (Throwable) {
      }
    }
