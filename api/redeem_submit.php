...
$LOG_FILE = dirname(__DIR__) . '/storage/logs/redeem.log';
...
function mask_redeem_code(string $code): string {
  // keep prefix 'ETHNIX-' & suffix '-XXXX', mask middle
  $c = trim($code);
  if ($c==='') return 'E********';
  if (preg_match('~^(ETHNIX-|BABACHECKER-)(.+)(-CREDITS|-FREE|-PREMIUM)$~i', $c, $m)) {
    $body = $m[2];
    $keepL = 4; $keepR = 3;
